...
class template {
    ...
    function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
        $basefile = basename(LCL_ROOT . $tplfile, '.htm');
        $file == 'common/header' && defined('CURMODULE') && CURMODULE && $file = 'common/header_' . CURMODULE;
        $this->file = $file;

        if ($fp = @fopen(LCL_ROOT . $tplfile, 'r')) {
            $template = @fread($fp, filesize(LCL_ROOT . $tplfile));
            fclose($fp);
        } elseif ($fp = @fopen($filename = substr(LCL_ROOT . $tplfile, 0, -4) . '.php', 'r')) {
            $template = $this->getphptemplate(@fread($fp, filesize($filename)));
            fclose($fp);
        } else {
            $tpl = $tpldir . '/' . $file . '.htm';
            $tplfile = $tplfile != $tpl ? $tpl . ', ' . $tplfile : $tplfile;
            $this->error('template_notfound', LCL_ROOT . $tplfile);
        }
        ...
        $template = "<? if(!defined('IN_LCL')) exit('Access Denied'); {$headeradd}?>\n$template";
        ...
        if (!@$fp = fopen(LCL_ROOT . $cachefile, 'w')) {
            $this->error('directory_notfound', dirname(LCL_ROOT . $cachefile));
        }
    ...
    function languagevar($var) {
        ...
        if (!isset($langvar[$var])) {
            $this->language['inner'] = lang('default');
            if (!$isplugin) {
                ...
                foreach (lang(LCL_ROOT . '/language') as $k => $v) {
                ...
                if (empty($_G['config']['plugindeveloper'])) {
                    ...
                    if (@include(LCL_ROOT . './data/plugindata/' . $vars[0] . '.lang.php')) {
    ...
    function loadcsstemplate() {
        ...
        $content = @implode('', file(LCL_ROOT . './data/cache/style_' . STYLEID . '_module.css'));
        ...
        if ($this->csscurmodules) {
            ...
            if (@$fp = fopen(LCL_ROOT . './data/cache/style_' . STYLEID . '_' . $_G['basescript'] . '_' . CURMODULE . '.css', 'w')) {
    ...
    function error($message, $tplname) {
        $msg = '<div style="color:#F00">' . $message . ' file:' . $tplname . '</div> ';
        runlog("errorlog", $msg);
        lcl_error::template_error($message, $tplname);
    }
