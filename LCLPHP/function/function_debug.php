...
function debugmessage($ajax = 0) {
    ...
    global $_G;
    $debugfile = '_debug.php';
    $akey = md5(@TIME);
    if (!defined('LCL_DEBUG') || !LCL_DEBUG || defined('IN_ARCHIVER') || defined('IN_MOBILE')) {
        return;
    }
    $phpinfok = 'I';
    $viewcachek = 'C';
    ...
    require_once LCL_ROOT . './lcl_version.php';
    ...
    foreach ($db->sqldebug as $string) {
        ...
        foreach ($string[2] as $error) {
            $error['file'] = str_replace(array(LCL_ROOT, '\\'), array('', '/'), $error['file']);
            ...
            if (strexists($error['file'], 'lcl/lcl_table') || strexists($error['file'], 'table/table')) {
                $dt = ' &bull; ' . $error['file'];
                $lcl_table++;
            }
        }
    ...
    if ($ajax) {
        ...
        file_put_contents(LCL_ROOT . './' . $ajaxhtml, $sqldebug, FILE_APPEND);
        return;
    }
    file_put_contents(LCL_ROOT . './' . $ajaxhtml, '<?php if(empty($_GET[\'k\']) || $_GET[\'k\'] != \'' . $akey . '\') { exit; } ?><style>body,table { font-size:12px; }table { width:90%;border:1px solid gray; }</style><a href="javascript:;" onclick="location.href=location.href">Refresh</a><br />');
    foreach ($sqlw as $k => $v) {
        $sqlw[$k] = $k . ': ' . $v;
    }
    $sqlw = '(' . ($lcl_table ? 'lcl_table: ' . $lcl_table . ($sqlw ? ', ' : '') : '') . ($sqlw ? '<s>' . implode(', ', $sqlw) . '</s>' : '') . ')';
...
elseif(isset($_GET[\'' . $mysqlplek . '\'])) {
    ...
    if (!defined('IN_ADMINCP') && file_exists(LCL_ROOT . './template/admincp/res/img/temp-grid.png'))
        $debug .= <<<EOF
...
s += '<div id="tempgrid" style="position: absolute; top: 0px; left: 50%; margin-left: -500px; width: 1000px; height: 0; background: url(admincp/res/img/temp-grid.png); visibility :hidden;"></div>';
...
EOF;
    ...
    $svn = '';
    if (file_exists(LCL_ROOT . './.svn/entries')) {
        $svn = @file(LCL_ROOT . './.svn/entries');
        $time = $svn[9];
    ...
    $debug .= '
		...
		#__debug_c_4_nav { background:#FFF; border:1px solid black; border-top:none; padding:5px; position: fixed; top:0px; right:0px }
            ...
            '<em>版本:</em> LCLPHP! ' . LCL_VERSION . ($svn ? $svn : ' ' . LCL_RELEASE) . '<br />' .
            ...
            '<img src="../template/admincp/res/img/arw_r.gif" /><a id="__debug_5" href="#debugbar" onclick="switchTab(\'__debug\', 5, ' . $max . ')">$_COOKIE</a>' .
            ($_G['adminid'] == 1 ? '<img src="../template/admincp/res/img/arw_r.gif" /><a id="__debug_6" href="#debugbar" onclick="switchTab(\'__debug\', 6, 6)">$_G</a>' : '') .
            ($_G['adminid'] == 1 ?
                    '<img src="../template/admincp/res/img/arw_r.gif" /><a href="' . $debugfile . '?k=' . $akey . '&' . $phpinfok . '" target="_blank">phpinfo()</a>' .
                    '<img src="../template/admincp/res/img/arw_r.gif" /><a href="' . $debugfile . '?k=' . $akey . '&' . $mysqlplek . '" target="_blank">MySQL 进程列表</a>' .
                    '<img src="../template/admincp/res/img/arw_r.gif" /><a href="' . $debugfile . '?k=' . $akey . '&' . $viewcachek . '" target="_blank">查看缓存</a>' .
                    '<img src="../template/admincp/res/img/arw_r.gif" /><a href="../misc.php?mod=initsys&formhash=' . formhash() . '" target="_debug_initframe" onclick="parent.document.getElementById(\'_debug_initframe\').onload = function () {parent.location.href=parent.location.href;}">更新缓存</a>' : '') .
            '<img src="../template/admincp/res/img/arw_r.gif" /><a href="../install/update.php" target="_blank">执行 update.php</a>' .
            '</table>' .
            '</div>' .
    ...
    $debug .= '</ol></div>' .
            ...
            '<div id="__debug_c_2" style="display:none"><b>IP: </b>' . $_G['clientip'] . '<br /><b>User Agent: </b>' . $_SERVER['HTTP_USER_AGENT'] . '<br /><b>BROWSER.x: </b><script>for(BROWSERi in BROWSER) {var __s=BROWSERi+\':\'+BROWSER[BROWSERi]+\' \';document.getElementById(\'__debug_b\').innerHTML+=BROWSER[BROWSERi]!==0?__s:\'\';document.write(__s);}</script></div>' .
            '<div id="__debug_c_3" style="display:none"><ol>';
    foreach ($includes as $fn) {
        $fn = str_replace(array(LCL_ROOT, "\\"), array('', '/'), $fn);
        $debug .= '<li>';
        if (@preg_match('/^/plugin/', $fn)) {
            $debug .= '[插件]';
    ...
    $fn = 'data/' . $debugfile;
    file_put_contents(LCL_ROOT . './' . $fn, $debug);
