...
function updatecache($cachename = '') {
    ...
    if (!$updatelist) {
        ...
        build_cache_setting();
        $cachedir = LCL_ROOT . './function/cache';
...
function writetocache($script, $cachedata, $prefix = 'cache_') {
    ...
    $dir = LCL_ROOT . './data/sysdata/';
    if (!is_dir($dir)) {
        dmkdir($dir, 0777);
    }
    if ($fp = @fopen("$dir$prefix$script.php", 'wb')) {
        fwrite($fp, "<?php\n//LCL cache file, DO NOT modify me!\n//Identify: " . md5($prefix . $script . '.php' . $cachedata) . "\n\n$cachedata?>");
        fclose($fp);
    } else {
...
function cleartemplatecache() {
    $tpl = dir(LCL_ROOT . './data/template');
    while ($entry = $tpl->read()) {
        if (preg_match("/\.tpl\.php$/", $entry)) {
            @unlink(LCL_ROOT . './data/template/' . $entry);
        }
    }
    $tpl->close();
}
