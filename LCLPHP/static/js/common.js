...
function get_category(id, next, select_id) {
    var url = siteurl + '/ajax.php?action=get_category&parent_id=' + id;
    $.ajax({
        type: "GET",
        url: url,
        error: function(data) {
        ...
        success: function(v) {
            v = "<option value=''>请选择商品分类</option>" + v;
            $('#' + next).empty().html(v);
 ...
 * @path  string 指定上传保存文件夹,默认存在data/attachment/goods/目录
 * @callback string  回调函数(单张图片返回保存路径字符串，多张则为路径数组 )
 */
function GetUploadify(num, elementid, path, callback)
{
    var upurl = siteurl + '/static/js/uploadify/index.php?action=Uploadify&a=upload&num=' + num + '&input=' + elementid + '&path=' + path + '&func=' + callback;
...
function showMenu(v) {
    ...
    if (cover)
        document.getElementById(menuid + '_cover').style.display = '';
    if (fade) {
        var O = 0;
        ...
        var hide = function() {
            if (menuObj.cache) {
                if (menuObj.style.visibility != 'hidden') {
                    ...
                    if (menuObj.cover)
                        document.getElementById(attr + '_cover').style.display = 'none';
                }
            } else {
                menuObj.parentNode.removeChild(menuObj);
                if (menuObj.cover)
                    document.getElementById(attr + '_cover').parentNode.removeChild(document.getElementById(attr + '_cover'));
            }
            var tmp = [];
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
    ...
    try {
        if (document.getElementById('fwin_dialog_submit'))
            document.getElementById('fwin_dialog_submit').focus();
    } catch (e) {
    }
}
...
function initSearchmenu(searchform, cloudSearchUrl) {
    ...
    var searchtxt = document.getElementById(searchform + '_txt');
    if (!searchtxt) {
        searchtxt = document.getElementById(searchform);
    }
    ...
    if (!document.getElementById(searchform + '_type_menu'))
        return false;
    var o = document.getElementById(searchform + '_type');
    var a = document.getElementById(searchform + '_type_menu').getElementsByTagName('a');
    var formobj = searchtxt.form;
    for (var i = 0; i < a.length; i++) {
        if (a[i].className == 'curtype') {
            o.innerHTML = a[i].innerHTML;
            document.getElementById(searchform + '_mod').value = a[i].rel;
            formobj.method = 'post';
            if ((a[i].rel == 'forum' || a[i].rel == 'curforum') && defaultUrl != cloudSearchUrl) {
                formobj.action = cloudSearchUrl;
                formobj.method = 'get';
                if (document.getElementById('srchFId')) {
                    document.getElementById('srchFId').value = a[i].rel == 'forum' ? 0 : a[i].getAttribute('fid');
                }
            } else {
        ...
        a[i].onclick = function() {
            ...
            document.getElementById(searchform + '_mod').value = this.rel;
            formobj.method = 'post';
            if ((this.rel == 'forum' || this.rel == 'curforum') && defaultUrl != cloudSearchUrl) {
                formobj.action = cloudSearchUrl;
                formobj.method = 'get';
                if (document.getElementById('srchFId')) {
                    document.getElementById('srchFId').value = this.rel == 'forum' ? 0 : this.getAttribute('fid');
                }
            } else {
                formobj.action = defaultUrl;
            }
...
function searchFocus(obj) {
    ...
    if (document.getElementById('cloudsearchquery') != null) {
        document.getElementById('cloudsearchquery').value = obj.value;
    }
}
...
function navShow(id) {
    var mnobj = document.getElementById('snav_mn_' + id);
    if (!mnobj) {
        return;
    }
    var uls = document.getElementById('mu').getElementsByTagName('ul');
    ...
    if (mnobj.className != 'cl current') {
        ...
        mnobj.style.width = (document.getElementById('nv').clientWidth) + 'px';
        mnobj.style.display = '';
    }
}
...
function patchNotice() {
    if (document.getElementById('patch_notice')) {
        ajaxget('misc.php?mod=patch&action=patchnotice', 'patch_notice', '');
    }
}

function pluginNotice() {
    if (document.getElementById('plugin_notice')) {
        ajaxget('misc.php?mod=patch&action=pluginnotice', 'plugin_notice', '');
    }
}

function ipNotice() {
    if (document.getElementById('ip_notice')) {
...
function showfocus(ftype, autoshow) {
    var id = parseInt(document.getElementById('focuscur').innerHTML);
    if (ftype == 'prev') {
    ...
    document.getElementById('focuscur').innerHTML = id;
    document.getElementById('focus_con').innerHTML = document.getElementById('focus_' + (id - 1)).innerHTML;
}
...
function rateStarHover(target, level) {
    if (level == 0) {
        ...
        document.getElementById(target).style.width = level * 16 + 'px';
    }
}
function rateStarSet(target, level, input) {
    document.getElementById(input).value = level;
    document.getElementById(target).className = 'star star' + level;
}
...
function checkBlind() {
    var dom = document.getElementById('switchblind');
    if (dom) {
        if (loadUserdata('is_blindman')) {
            dom.title = '关闭辅助访问';
        } else {
...
function getElementOffset(element) {
    ...
    if (document.getElementById('nv').style.position == 'fixed') {
        top -= parseInt(document.getElementById('nv').style.height);
    }
