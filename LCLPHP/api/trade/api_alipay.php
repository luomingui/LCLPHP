...
define('LCL_PARTNER', $ec_partner);
define('LCL_SECURITYCODE', $ec_securitycode);
define('LCL_DIRECTPAY', $ec_creditdirectpay);
...
function trade_returnurl($args) {
    ...
    $sign = substr($sign, 1);
    $sign = md5($sign . LCL_SECURITYCODE);
...
function trade_notifycheck($type) {
    ...
    unset($notify['diy']);
    if (dfsockopen("http://notify.alipay.com/trade/notify_query.do?partner=" . LCL_PARTNER . "&notify_id=" . $notify['notify_id'], 60) !== 'true') {
        exit('Access Denied');
    }
    ...
    if ($type == 'trade') {
        ...
        if ($notify['sign'] != md5(substr($sign, 1) . LCL_SECURITYCODE)) {
            exit('Access Denied');
        }
    }

    if (($type == 'credit' || $type == 'invite') && (!LCL_DIRECTPAY && $notify['notify_type'] == 'trade_status_sync' && ($notify['trade_status'] == 'WAIT_SELLER_SEND_GOODS' || $notify['trade_status'] == 'TRADE_FINISHED') || LCL_DIRECTPAY && ($notify['trade_status'] == 'TRADE_FINISHED' || $notify['trade_status'] == 'TRADE_SUCCESS')) || $type == 'trade' && $notify['notify_type'] == 'trade_status_sync') {
        return array(
            'validator' => TRUE,
            'status' => trade_getstatus(!empty($notify['refund_status']) ? $notify['refund_status'] : $notify['trade_status'], 1),
            'order_no' => $notify['out_trade_no'],
            'price' => !LCL_DIRECTPAY && $notify['price'] ? $notify['price'] : $notify['total_fee'],
