...
define('LCL_PARTNER', $_G['setting']['ec_tenpay_bargainor']);
define('LCL_SECURITYCODE', $_G['setting']['ec_tenpay_key']);
define('LCL_AGENTID', '1204737401');

define('LCL_TENPAY_OPENTRANS_CHNID', $_G['setting']['ec_tenpay_opentrans_chnid']);
define('LCL_TENPAY_OPENTRANS_KEY', $_G['setting']['ec_tenpay_opentrans_key']);
...
function credit_payurl($price, &$orderid, $bank = 'DEFAULT') {
    include_once LCL_ROOT . './source/class/class_chinese.php';
    global $_G;

    $date = dgmdate(TIMESTAMP, 'YmdHis');
    $suffix = dgmdate(TIMESTAMP, 'His') . rand(1000, 9999);
    $transaction_id = LCL_PARTNER . $date . $suffix;
    ...
    $reqHandler->init();
    $reqHandler->setKey(LCL_SECURITYCODE);

    $reqHandler->setParameter("partner", LCL_PARTNER);
    ...
    $reqHandler->setParameter("agentid", LCL_AGENTID);
...
function trade_payurl($pay, $trade, $tradelog) {
    ...
    $key = LCL_TENPAY_OPENTRANS_KEY;

    $chnid = LCL_TENPAY_OPENTRANS_CHNID;
...
function trade_notifycheck($type) {
    ...
    if ($type == 'credit' || $type == 'invite') {
        ...
        $resHandler->setKey(LCL_SECURITYCODE);

        $resHandler->setParameter("bankname", "");
    } else {
        if (!LCL_TENPAY_OPENTRANS_KEY) {
            exit('Access Denied');
        }
        $resHandler = new MediPayResponseHandler();
        $resHandler->setKey(LCL_TENPAY_OPENTRANS_KEY);
    }
    if ($type == 'credit' || $type == 'invite') {
        if ($resHandler->isTenpaySign() && LCL_PARTNER == $_GET['partner']) {
            return array(
